...
import resultBackground from '../assets/backgrounds/intro-bg.png'; // NOVO: Importa o fundo do resultado
...
function QuizScreen({ onRestart }) {
  ...
  // NOVO ESTADO para controlar se a digitação foi pulada (necessário se você manteve a funcionalidade de skip)
  ...
  // O texto exibido é o completo se foi pulado, ou o animado
  ...
  // Determina a classe do container
  const containerClass = quizFinished ? "app-container result-screen" : "app-container";

  // NOVO: Define o estilo de fundo condicionalmente
  ...
  return (
    <div
      className={containerClass} // Usa a classe condicional
      style={backgroundStyle} // Usa o estilo de fundo condicional
    >
      {quizFinished ? (
        // Na tela de resultado, usa a classe result-box para centralização
        <DialogueBox
